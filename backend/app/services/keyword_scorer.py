...
# 언론(뉴스)은 대개 사실 보도라 조장 위험이 낮지만, 병역 키워드가 많아 점수가 높게
# 나와 일반 임계치(2)로는 거의 안 걸러진다. 라벨 데이터(언론 170건) 실측 결과
# 중간·높음 언론의 최소 점수가 4 → 임계치 4까지는 중간·높음을 하나도 놓치지 않는다.
# (5 이상은 중간·높음을 잘못 스킵). 사전 재생성/데이터 변동 시 재검증할 것.
NEWS_PREFILTER_THRESHOLD = 4
