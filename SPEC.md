...
| 009_keyword_tuning.sql | 뉴스·지식인 키워드를 사건성·조합형으로 교체 |
| 010_verify.sql | 2단계 검증 컬럼(`content_full`·`verify_*`) + `run_logs`에 `verify` 허용 |
...
backend/
  app/
    ...
    services/
      ...
      text_normalize.py        난독 해제 (NFKC·zero-width·구분자) — 사전필터 전처리
...
**출처와 무관하게 임계치는 하나(`PREFILTER_THRESHOLD = 3`)만 쓴다** — 요구 Q7
("언론·SNS·커뮤니티·유튜브 등 출처별로 판단 기준이나 판단 방식은 동일하게 적용").

| 검증 | 스크립트 | 결과 (2026-08-07) |
|------|---------|------|
| 임계치 산정 | `validate_prefilter.py` | 사람 라벨 6,430건 — 임계치 2에서 조장 미스율 0.34%, 3에서 6.17% |
| 언론 구간 안전성 | `validate_news_prefilter.py` | 분류된 언론 218건 — 무손실 상한 **7**, 설정값 3은 **안전** |
...
> 2026-08-05 제거하고 단일 임계치로 되돌렸다. (그 뒤 실수집 676건 분석으로 2 → 3 상향.)

### 난독 해제 (`text_normalize.py`) — 2026-08-07

게이트와 사전필터가 **원문 문자열만 비교**해서, `병.역.면.제`·`병 역 면 제`·zero-width
삽입 글이 게이트를 넘지 못하고 `action_type='비대상'`으로 자동 확정된 뒤 Gemini에 가지
않았다. 조장 게시물이 난독화를 가장 많이 쓰는 집단이라 **누락 경로**였다.

매칭을 **원문 ∪ `normalize()` ∪ `compact()`** 합집합으로 바꿨다. 적용 범위는 오탐 비용이
달라 게이트와 사전이 다르다 — 근거는 `keyword_scorer.py` 상단 주석.

| 대상 | `compact()` 적용 | 실측 |
|------|------|------|
| 게이트(31개) | **전 용어** | 통과율 63.2%→63.5%(+0.27%p), 경계 오탐 1건/1,485 |
| 사전(493개) | **길이 3 이상만** | 3음절+ 오탐 0/60건 · 2음절 이하는 대부분 경계 오탐 |

사람 라벨 6,430건 대조: 점수 상승 216건(3.36%, `신체 검사`·`군 면제` 등 띄어쓰기 변형),
**임계치 3 스킵 판정 뒤집힘 0건** — 미스율 불변. 실이득은 난독화가 실제로 나오는 라이브
수집에서 발생한다.

> 대가로 `그러면 제가`→`면제` 같은 어절 경계 오탐이 게이트를 통과한다. 게이트 오탐은
> Gemini 호출 1건이고 누락은 조장 글 유실이라 비대칭이 커서 감수했다. `tests/test_prefilter.py`
> 가 이 동작을 고정하고 있으므로, 깨지면 설계가 바뀐 것이니 근거부터 확인할 것.
> **라이브 수집에서의 실제 증가폭은 아직 미측정** — `docs/다음_할일.md` 7번.

> 출처: 데이터분석팀 `safewatch-classifier`의 `src/safewatch/io/normalize.py`. 표준
> 라이브러리만 쓰며 모델·데이터는 가져오지 않았다.

### 2단계 검증 — 삭제대상을 본문으로 재확인 (2026-08-07)

수집은 네이버 검색 API의 **요약(`description`, 약 120자)**만 저장한다([naver.py](backend/app/crawlers/naver.py#L74)).
2026-08-07 실측 — 지식인 본문 중앙값 128자, **삭제대상 38건은 전부 200자 미만**이었다.
그 요약만 보고 브로커로 판정한 글의 답변 전문이 병무용 진단서 제도 안내였고 "전화번호"는
**지방병무청 상담번호 14개 목록**이었던 사례가 확인됐다.

삭제 요청 후보(`삭제대상`·`종합판단`)만 본문을 다시 조회해 재판정한다.
**원 분류 컬럼은 덮어쓰지 않고** `verify_*`에 따로 쌓아 검수 화면에서 나란히 보여준다 —
덮어쓰면 왜 바뀌었는지 추적할 수 없고 누적분과 기준이 어긋나 분기 보고서가 왜곡된다.

| 출처 | 본문 조회 | 비고 |
|------|:--:|------|
| 지식인 | ⭕ | 직접 조회 |
| SNS(블로그) | ⭕ | iframe → `PostView.naver` 2단계 |
| 언론 | ⭕ | 매체별 선택자 + 최장 블록 폴백 |
| 커뮤니티(카페) | ❌ | 네이버 카페 **로그인 벽** |
| 유튜브 | — | 댓글·영상설명이 이미 전문 |

- 모듈: `app/crawlers/fulltext.py` · `app/services/verify.py` · `app/api/verify.py`
- API: `POST /api/verify/run` · `GET /api/verify/status` · `/verify/pending` · `/verify/results`
- 화면: 「검수/선정」 탭에 실행 패널 + 1차/2차 대조

> ⚠️ **한계**: 1차에서 비대상으로 떨어진 글은 보지 않는다. **오탐만 줄고 누락은 그대로다.**
> 누락 규모는 `estimate_snippet_miss.py`로 표본 추정한다(비대상 모집단에서 층화 무작위
> 표본 → 본문 재판정 → Wilson 신뢰구간).
>
> ⚠️ **본문 조회는 공식 API가 아니다.** 렌더링된 페이지 조회이며 디시·에펨 스크래핑과
> 같은 범주, **D1 법무·정보보안 검토 대상**이다. 그래서 전건이 아니라 삭제대상·표본으로
> 범위를 좁혔다.

### 정책비판 오탐 방지 (2026-08-07)

2026-08-07 실측 — 삭제대상 38건 중 **27건이 정치 비판 유튜브 댓글**이었다. 원인은 축 B
카테고리 정의의 충돌이다.

- `편법·속임수·공정성 훼손`: "…**병무행정 신뢰도 훼손**" → 삭제대상
- `정책비판`: "병무정책 비판·패러디·풍자(허위 없음)" → 비대상

정치 비판은 둘 다 만족해서 앞쪽이 뒤쪽을 삼켰다. **27건 중 6건은 Gemini가
`intent_type='풍자/비판'`이라 판정했는데도** `CATEGORY_TO_ACTION` 결정적 매핑이 삭제대상을
확정했다.

조치 두 가지:
1. **프롬프트** — `편법·속임수·공정성 훼손`을 *규정 회피·거짓 신청 서술이 있을 때*로 한정,
   정치인·정부 비판 사례 3종 추가, 의도-구분 일관성 규칙 명시
2. **파서 안전망** — `intent_type='풍자/비판'` + `action_type='삭제대상'`이면 **종합판단으로 강등**
   (비대상으로 낮추지 않는 이유: 풍자 형식의 실제 조장 글이 있을 수 있다)

부수 효과로 **알림 오발송도 막힌다**. `DELETE_SCORE_FLOOR = 67`이 삭제대상 점수를 67로
밀어올리는데 `alert_settings.min_score`도 67이라, **삭제대상은 자동으로 알림 대상**이었다.
종합판단이면 하한이 걸리지 않아 Gemini 원점수(신뢰저하 구간 50~70)에 머문다.
