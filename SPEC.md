...
| 대상 | 임계치 | 검증 스크립트 | 근거 (2026-08-05 재검증) |
|------|--------|--------------|------|
| 일반 소스 | 2 | `validate_prefilter.py` | 사람 라벨 6,430건 — 미스율 0.34%, 스킵 0.4% |
| **언론** | 4 | `validate_news_prefilter.py` | 분류된 언론 218건 — 위험 미스 0%, 스킵 3.7% |

두 값 모두 재검증에서 코드 설정값과 **일치**했다(권장 2 / 권장 4).

> ⚠️ **언론 임계치는 요구사항 Q7("출처별 판단 기준 동일 적용")과 상충한다.**
> 재검증 결과 언론 완화의 실익은 **호출 3.7% 절감**뿐이다(임계치 2로 낮춰도 2.3% 절감이라
> 차이는 1.4%p). 절감 폭이 작으므로, 감사 대응을 우선한다면 언론도 2로 되돌려
> 상충을 없애는 편이 낫다. — 유지/변경은 사용자 결정 사항.
>
> 언론 검증은 사람 라벨이 아니라 **Gemini 분류 결과를 정답으로 쓴다**(라벨 자료에 출처
> 구분이 없기 때문). 일반 임계치만큼 강한 근거가 아니므로 올릴 때는 보수적으로 볼 것.
